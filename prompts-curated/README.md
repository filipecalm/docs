# Prompts curados

Coleção de prompts extraídos de fontes externas (screenshots, redes sociais), separados dos prompts de engenharia em [prompts.md](../prompts.md).

## Navegação por tipo

**[por-tipo.md](./por-tipo.md)** — índice único por finalidade.

| Pasta | Conteúdo |
|-------|----------|
| [youtube/](./youtube/) | 10 prompts YouTube + templates |
| [instagram/](./instagram/) | 7 prompts conta faceless |
| [conteudo/](./conteudo/) | Ideias de nicho (15 tópicos) |
| [produto-digital/](./produto-digital/) | eBooks e infoprodutos |
| [carreira/](./carreira/) | Emprego remoto, entrevistas, LinkedIn |
| [aprendizado/](./aprendizado/) | Estudo, DeepSeek, idiomas, Feynman |
| [desenvolvimento/](./desenvolvimento/) | Co-Fundador Técnico, auditoria de segurança |
| [financas/](./financas/) | Orçamento e cortes |
| [concurso/](./concurso/) | CESGRANRIO Banco do Brasil |
| [imagem/](./imagem/) | Upscale e melhoria de imagem |
| [comportamento-agente/](./comportamento-agente/) | Pensamento crítico, reflexão |
| [legado/](./legado/) | Tutoriais, roadmaps e desafios (não são prompts) |
| [origens/](./origens/) | Catálogos brutos por pasta de screenshots |

## Índice por origem

| Arquivo | Origem no disco |
|---------|-----------------|
| [origens/from-tops.md](./origens/from-tops.md) | `Desktop/Filipe/tops` |
| [origens/from-salvas.md](./origens/from-salvas.md) | `Pictures/Salvas` |
| [origens/from-screenshots.md](./origens/from-screenshots.md) | `Pictures/Screenshots` (2025–2026) |
| [legado/tutoriais-roadmaps.md](./legado/tutoriais-roadmaps.md) | `Pictures/Screenshots` (2022–2024) |

## Templates reutilizáveis (por pasta)

Cada pasta de tipo contém os prompts com placeholders padronizados — no mesmo espírito de [prompts-generic/](../prompts-generic/).

| Pasta | Arquivos principais |
|-------|---------------------|
| [youtube/](./youtube/) | pacote-completo, workflow-sem-rosto, ideias-virais, titulos-virais, roteiro-magnetico |
| [instagram/](./instagram/) | crescimento-faceless |
| [aprendizado/](./aprendizado/) | produtividade |
| [desenvolvimento/](./desenvolvimento/) | co-fundador-tecnico, auditoria-seguranca |
| [financas/](./financas/) | orcamento-e-cortes |
| [concurso/](./concurso/) | cesgranrio-bb |
| [imagem/](./imagem/) | melhoria-imagem |
| [comportamento-agente/](./comportamento-agente/) | pensamento-critico |
