# Prompts curados — pasta `Screenshots`

Prompts extraídos de `C:\Users\filip\Pictures\Pictures\Screenshots` (997 imagens), **agrupados por tipo**.

> **Índice geral por tipo:** [por-tipo.md](../por-tipo.md)  
> **Tutoriais / roadmaps / desafios (2022–2024):** [legado/tutoriais-roadmaps.md](../legado/tutoriais-roadmaps.md)  
> **Overlap Salvas:** [from-salvas.md](./from-salvas.md)

## Resumo por tipo

| Tipo | Qtd. | Template |
|------|------|----------|
| Conteúdo e redes sociais | 8 | [instagram/crescimento-faceless.md](../instagram/crescimento-faceless.md) |
| Produto digital | 2 | [produto-digital/](../produto-digital/) |
| Carreira e emprego | 2 | [carreira/](../carreira/) |
| Aprendizado | 1 | [aprendizado/](../aprendizado/) |
| Desenvolvimento | 1 | [desenvolvimento/co-fundador-tecnico.md](../desenvolvimento/co-fundador-tecnico.md) |
| Comportamento do agente | 5 instruções | [comportamento-agente/pensamento-critico.md](../comportamento-agente/pensamento-critico.md) |

---

## Conteúdo e redes sociais

### Instagram — @euthayboss (7)

**Imagens:** `Screenshot_20260204-192000.png` … `192033.png`  
**Template:** [instagram/crescimento-faceless.md](../instagram/crescimento-faceless.md)

```
1. Liste 10 nichos de produtos digitais com baixa concorrência, alta demanda e potencial de renda passiva.

2. Para [nicho escolhido], me dê 20 ideias de reels viral ou carrossel que eu possa recriar sem mostrar meu rosto.

3. Escreva uma legenda do Instagram em estilo viral com um gancho forte e CTA para este post: [cole sua ideia de conteúdo].

4. Gere 15 hashtags de alto desempenho e 5 textos de gancho de parada de rolagem para minha conta do instagram sobre [nicho].

5. Construa um calendário de conteúdo de 30 dias para minha conta nova, otimizada para engajamento e crescimento.

6. Liste 5 maneiras de monetizar uma conta nova do Instagram sobre [nicho], incluindo afiliados, programas de assinaturas e produtos digitais.

7. Me mostre como postar conteúdos em massa, agendar postagens e automatizar legendas para que minha conta seja executada no piloto automático.
```

### YouTube — canal de receitas (1)

**Fonte:** @Yourchatgptguide · `Screenshot_20250103-015853.png`

```
I'm starting a YouTube channel called "Marks Eats", help me find some new viral recipes to cook and create a 5 video per week schedule for me to use this month.
```

---

## Produto digital e monetização

### eBook comercial — carlos.sousasoares

**Imagens:** `Screenshot_20250519-135958.png`, `140004.png`

```
Atue como um estrategista digital especializado na criação de eBooks que geram vendas.

Preciso que você desenvolva um eBook completo, pronto para captar leads e faturar.

Incluir:
- Título chamativo e comercial
- Subtítulo que destaque o principal benefício
- Estrutura de capítulos lógica e envolvente
- Texto dos capítulos com exemplos práticos aplicáveis
- Sugestão de capa para designer ou IA
- Chamada para ação no final, levando para minha oferta digital
- Tom de escrita: leve, persuasivo e voltado para conversão

Informações base:
- Tema: [Especifique aqui]
- Público-alvo: [Especifique aqui]
- Objetivo: [Capturar leads, vender, gerar autoridade etc.]
- Extensão: [Ex: 3.000 a 5.000 palavras]
```

### eBook por nicho — ofcsuelen (Canal Dark)

**Imagem:** `Screenshot_20260129-233554.png`

```
Sua tarefa é criar um E-book completo e detalhado sobre o tema: [SEU NICHO]. Que não se limite apenas a títulos ou capítulos, mas que contenha conteúdo aprofundado em cada parte de forma estruturada e prática, para entregar valor ao leitor. O E-book deve ser educativo, envolvente incluindo passo a passo, com explicações claras, exemplos e ações aplicáveis.
```

---

## Carreira e emprego

### Análise de requisitos — Ruby/React

**Imagem:** `Screenshot_20250307-111006.png`

```
Sou um programador e estou procurando as palavras-chave relacionadas as vagas da stack de Ruby e React. Meu objetivo é saber os requisitos das vagas que mais se repetem e quantas vezes aquele requisito se repete para eu poder estudá-los.

Com base nos Job Descriptions abaixo, me mostre uma lista com esses requisitos, me mostrando quantas vezes se repetiram e ordenando-os em ordem decrescente com base na quantidade de vezes que se repetiram.

Só considere requisitos que se repetiram pelo menos 2 vezes, para cima.

[COLE OS JOB DESCRIPTIONS AQUI]
```

### LinkedIn — reflexão (Tammy)

**Imagem:** `Screenshot_20250426-122916.png`

```
Given our previous interactions, what is 1 thing you think I hide from my community on LinkedIn but I should tell them? Respond to it in BR portuguese, please.
```

---

## Aprendizado e estudo

### Técnica Feynman

**Imagem:** `Screenshot_20230511-062848.png` · também em [legado/tutoriais-roadmaps.md](../legado/tutoriais-roadmaps.md)

```
Explique [TÓPICO OU HABILIDADE] da forma mais simplificada possível como se estivesse ensinando a um iniciante completo.
```

---

## Desenvolvimento e engenharia

### Co-Fundador Técnico — vibe coding

**Fonte:** Miles Deutscher (AIEDGE) · Alexandre Messina · `Screenshot_20260211-184524.png`  
**Template:** [desenvolvimento/co-fundador-tecnico.md](../desenvolvimento/co-fundador-tecnico.md)

Prompt em 5 fases (Descoberta → Entrega) com papel de co-fundador técnico. Texto completo no template.

---

## Comportamento do agente

### Pensamento crítico — Leo Candido

**Imagem:** `Screenshot_20250425-134623.png`  
**Template:** [comportamento-agente/pensamento-critico.md](../comportamento-agente/pensamento-critico.md)

1. Analise minhas suposições. O que estou tomando como verdade que pode não ser?
2. Apresente contrapontos. O que um cético bem informado diria em resposta?
3. Teste meu raciocínio. Minha lógica se sustenta ou tem falhas?
4. Ofereça outras perspectivas. Como essa ideia pode ser vista de outra forma?
5. Priorize a verdade, não a concordância. Se eu estiver errado, me corrija com clareza e explique o motivo.

---

## Também nesta pasta (ver from-salvas)

| Imagem | Tipo | Doc |
|--------|------|-----|
| `Screenshot_20241209-082125.png` | Carreira — emprego remoto (6) | [from-salvas.md](./from-salvas.md) |
| `Screenshot_20250201-072146.png` | Aprendizado — DeepSeek (7) | [from-salvas.md](./from-salvas.md) |
| `Screenshot_20250212-170022.png` | Comportamento — reflexão | [from-salvas.md](./from-salvas.md) |
| `Screenshot_20231122-133829.png` | Imagem — DALL-E | [legado/tutoriais-roadmaps.md](../legado/tutoriais-roadmaps.md) |

---

## Inventário (997 imagens)

| Faixa | Prompts | Tutoriais / roadmaps |
|-------|---------|----------------------|
| 2025–2026 | 14 imagens | [legado](../legado/) |
| 2022–2024 | 2 (+ Feynman) | 38 itens |

Ver [por-tipo.md](../por-tipo.md) para o catálogo completo.
